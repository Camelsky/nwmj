cc.Class({
    ...
    onLoad: function () {
        ...
        var t = this.node.getChildByName("ZhuanZhuanRule");
       // t.active = false;
        for(var i = 0; i < t.childrenCount; ++i){
            var n = t.children[i].getComponent("CheckBox");
            if(n != null){
    ...
    onCreateRoomTabChange(event,data){
        var allwanfa = [
            "zimojiacheng",
            "dianganghua",
            "wanfaxuanze",
        ...
        var self = this;
        allwanfa.forEach(function(item){
            console.log(item)
            self.node.getChildByName(item).active = false;
        });
        wanfa[data].forEach(function(item){
            self.node.getChildByName(item).active = true;
        });
